...
// Truncate transactions to within sDate/eDate 
Transactions.prototype.truncate = function(sDate, sPx, eDate, ePx) {
    var sPos = this.position(sDate);
    if (eDate) {
        var ePos = -this.position(eDate);
        var trans = new Transactions(this.transactions.filter(function(e){return e.date>sDate && e.date<=eDate;})).add(sDate,sPos,sPx,0).add(eDate,ePos,ePx,0).sort();
        return trans;
    } else  {
