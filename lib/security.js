...
Security.prototype.calculate = function(P, N) {
    var period = P+"d";
    try {
        var result = this.indicator.returns(P);
        var values = result.slice(- (N || 252));
        this.returns[period] = statistics.TimeSeries(this.indicator.dates, result);
...
Security.prototype.performance = function(start,end, t) {
    ...
    var ret = returns.reduce(function(total,curr) { return total*(1+curr); }, 1)-1;
    var vol = statistics.stdev(returns);
