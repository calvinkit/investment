...
Table.Thousand = function (digits) {
    return function (val, width) {
        if (val === undefined) return ''
        if (typeof val != 'number')
            throw new Error(String(val) + ' is not a number')
        ...
        var sign = val<0?"-":"";
        var intPart = parseInt(Math.abs(val).toFixed(digits), 10) + '';
        var j = intPart.length > 3 ? intPart.length % 3 : 0;
        if (val == 0) return "";
        var s = sign + (j?intPart.substr(0, j)+',':'')+intPart.substr(j).replace(/(\d{3})(?=\d)/g, '$1' + ',') + (digits>0 ? '.' + Math.abs(val - intPart).toFixed(digits).slice(digits) : '');
        return Table.padLeft(s, width)
    }
}
/**
